...
function loadChar() {
	...
	const totalUsers = document.getElementById('total-users').innerText.replace('Online: ', '');
	const totalMovies = document.getElementById('total-movies').innerText.replace('Online: ', '');
	const totalMoviesOff = document.getElementById('total-movies-off').innerText.replace('Offline: ', '');
	const totalUsersOff = document.getElementById('total-users-off').innerText.replace('Offline: ', '');
	new Chart(ctx, {
		type: 'doughnut',
		data: {
			labels: ['Users', 'Users-Offline'],
			datasets: [{
				label: 'Users-Offline',
				data: [totalUsers, totalUsersOff],
	...
	new Chart(ctx2, {
		type: 'doughnut',
		data: {
			labels: ['Movies', 'Movies-Offline'],
			datasets: [{
				label: 'Movies-Offline',
				data: [totalMovies, totalMoviesOff],
				backgroundColor: ['#36A2EB', '#FF6384'],
				hoverBackgroundColor: ['#36A2EB', '#FF6384'],
			}]
		},
	...
	function displayMovies(movies, page) {
		...
		$.each(paginatedMovies, function (index, movie) {
			...
			if (movie.role == 2) {
				...
				if (movie.role == 0) {
					vip = 'Off';
					listItem.append('<td >' + '<button value=' + (movie.user_id) + ' class="btn btn-danger btn-update-role">' + vip + '</button>' + '</td>');
				}
				if (movie.role == 1) {
					vip = 'On';
					listItem.append('<td >' + '<button value=' + (movie.user_id) + ' class="btn btn-success btn-update-role"  >' + vip + '</button>' + '</td>');
		...
		$('.btn-update-role').off('click').click(function () {
			var idnguoidung = $(this).val();

			updatenguoidungRole(idnguoidung);
			loadnguoidunglist();
		});
...
$(document).ready(function () {
	...
	getAPI('/animu/api/char', function (result) {
		...
		$('#total-users').text('Online: ' + data[0]);
		$('#total-movies').text('Online: ' + data[1]);
		getAPI('/animu/api/char2', function (result) {
			var data = result.split(',');
			$('#total-users-off').text('Offline: ' + data[0]);
			$('#total-movies-off').text('Offline: ' + data[1]);
...
function updatenguoidungRole(idnguoidung) {

	getAPI('/animu/api/update/role/nguoidung/' + idnguoidung, function (result) {
